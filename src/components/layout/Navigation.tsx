...
import { Home, Users, Video, MessageSquare, Calendar, User } from "lucide-react";
      ...
      <div className="flex justify-around md:flex-col md:h-full md:justify-start md:pt-8">
        {links.map((link) => (
          ...
          >
            ...
            <span className="text-xs mt-1 md:hidden font-heading">{link.label}</span>
            <span className="hidden md:block absolute left-full ml-2 px-2 py-1 bg-primary text-white text-sm rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap font-heading">
              {link.label}
            </span>
          </Link>
        ))}
